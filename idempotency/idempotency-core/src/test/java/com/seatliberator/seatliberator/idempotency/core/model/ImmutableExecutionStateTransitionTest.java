...
@DisplayName("ImmutableExecutionState - Transition")
public class ImmutableExecutionStateTransitionTest {
    @Test
    ...
    void PENDING에서_RUNNING으로_전이하면_실행_시작_시각과_attemptCount가_갱신된다() {
        var state = ImmutableExecutionState.pending();
        Instant tryStartAt = Instant.now();

        state = state.markRunning(tryStartAt);
    ...
    void EXECUTION_ERROR에서_RUNNING으로_재전이하면_attemptCount가_증가한다() {
        var state = ImmutableExecutionState.executionError(1, Instant.now());
        Instant retriedAt = Instant.now().plusSeconds(10);

        state = state.markRunning(retriedAt);
    ...
    void RUNNING에서_EXECUTION_ERROR로_전이하면_resolvedAt과_output은_비워진다() {
        ...
        state = state.markExecutionError();
