...
public class DefaultEventStoreTest {
    private DefaultEventStore createStore(int batchSize) {
        return new DefaultEventStore(new DefaultEventStateTransitionPolicy(), batchSize);
    }
    ...
    @Test
    @DisplayName("batchSize 는 1 이상이어야 한다")
    void constructor_whenBatchSizeLessThanOne_throws() {
        assertThrows(
                IllegalArgumentException.class,
                () -> new DefaultEventStore(new DefaultEventStateTransitionPolicy(), 0)
        );
    }
    ...
    void acceptAndClaim() {
        ...
        var claimed = store.claimBatch(EventFlow.INBOUND, claimedAt);
    ...
    void claimBatch_excludesAlreadyProcessing() {
        ...
        var first = store.claimBatch(EventFlow.INBOUND, firstClaimedAt);
        var second = store.claimBatch(EventFlow.INBOUND, secondClaimedAt);
    ...
    void claimBatch_includesFailedEvent() {
        ...
        var firstClaim = store.claimBatch(EventFlow.INBOUND, firstClaimedAt);
        store.reportFailed(firstClaim.getFirst().trace().eventId(), failedAt);

        var secondClaim = store.claimBatch(EventFlow.INBOUND, secondClaimedAt);
    ...
    void reportCompleted() {
        ...
        assertTrue(store.claimBatch(EventFlow.INBOUND, claimedAt.plusSeconds(10)).isEmpty());
    }
    ...
    void claimBatch_whenClaimedAtBeforeAcceptedAt_throws() {
        ...
        assertThrows(
                ...
                () -> store.claimBatch(EventFlow.INBOUND, claimedAt)
        );
    }
}
