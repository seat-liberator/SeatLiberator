...
public class EmbeddableDailyTimeSegmentTest extends AbstractDailyTimeSegmentTest<EmbeddableDailyTimeSegment> {
    @Override
    ...
    void create_with_start_nano_of_day_and_duration() {
        var range = EmbeddableDailyTimeSegment.of(START_NANO_OF_DAY, DURATION);
    ...
    void throw_exception_when_duration_is_not_positive() {
        assertThatDomainThrownBy(() -> EmbeddableDailyTimeSegment.of(START_NANO_OF_DAY, Duration.ZERO))
                .hasPositiveMessageFor("duration");

        assertThatDomainThrownBy(() -> EmbeddableDailyTimeSegment.of(START_NANO_OF_DAY, Duration.ofNanos(-1)))
                .hasPositiveMessageFor("duration");
    }
    ...
    void copy_from_daily_nano_range() {
        var range = EmbeddableDailyTimeSegment.from(create(START_NANO_OF_DAY, END_NANO_OF_DAY));
