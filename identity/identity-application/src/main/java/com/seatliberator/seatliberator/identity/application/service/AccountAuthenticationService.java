...
public class AccountAuthenticationService implements AccountAuthenticator {
    ...
    @Override
    public AuthContextEntry.Credential authenticate(AuthenticationCommand.Credential command) {
        return credentialAccountStore.findCredentialAuthByEmail(command.email())
                ...
                .map(auth -> new AuthContextEntry.Credential(
                        auth.userId(),
                        auth.email(),
                        auth.passwordHash()
                ))
    ...
    @Override
    public AuthContextEntry.Federated authenticate(AuthenticationCommand.Federated command) {
        return federatedAccountStore.findFederatedAuthByRegistrationIdAndProviderUserId(
                        command.registrationId(),
                        command.providerUserId()
                )
                .map(auth -> new AuthContextEntry.Federated(
                        auth.userId(),
                        auth.registrationId(),
                        auth.providerUserId()
                ))
